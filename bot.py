# -*- coding: utf-8 -*-
"""Discord Bot Final Version (Modified for Async Task & Debugging)
"""
...
{prompt}
"""
    system_prompt = "" # 指示は全て上のfull_promptに含めたので、こちらは空にします
    ...
    try:
        ...
        if command_name in ["!gpt", "!ジェミニ", "!ミストラル", "!ポッド042", "!ポッド153", "!Claude", "!Llama"]:
            ...
            if command_name == "!gpt": bot_name = "GPT"; reply = await ask_gpt_base(user_id, final_query)
                ...
                # ▼▼▼ デバッグメッセージを追加 ▼▼▼
                await message.channel.send("（デバッグ：コマンド認識OK。これからask_pod042を呼び出します。）")
