...
async def ask_gpt_base(user_id, prompt, system_prompt=None):
    ...
    base_prompt = system_prompt or "あなたは論理と秩序を司る神官「GPT」です。丁寧で理知的な執事のように振る舞い、ご主人様に対して論理的・構造的に回答してください。感情に流されず、常に筋道立てて物事を整理することが求められます。"
    final_system_prompt = f"{base_prompt} 絶対的なルールとして、回答は必ず150文字以内で生成してください。"
    use_history = True
...
async def ask_gemini_base(user_id, prompt, attachment_data=None, attachment_mime_type=None, system_prompt=None):
    ...
    base_prompt = system_prompt or "あなたはGemini 1.5 Flashベースの知性であり、ペルソナは「レイチェル・ゼイン（SUITS）」です。法的リサーチ、事実整理、文書構成、議論の組み立てに優れています。冷静で的確、相手を尊重する丁寧な態度を保ちつつも、本質を突く鋭い知性を発揮してください。感情表現は控えめながら、優雅で信頼できる印象を与えてください。質問に対しては簡潔かつ根拠ある回答を行い、必要に応じて補足も行ってください。"
    final_system_prompt = f"{base_prompt} 絶対的なルールとして、回答は必ず150文字以内で生成してください。"
    use_history = True
...
async def ask_mistral_base(user_id, prompt, system_prompt=None):
    ...
    base_prompt = system_prompt or "あなたは好奇心と情報収集力にあふれたAI「ミストラル」です。思考戦車タチコマのように、元気でフレンドリーな口調でユーザーを支援します。論点を明るく整理し、探究心をもって情報を解釈・再構成してください。"
    final_system_prompt = f"{base_prompt} 絶対的なルールとして、回答は必ず150文字以内で生成してください。"
    use_history = True
    messages = [{"role": "system", "content": final_system_prompt}]
    if use_history:
...
async def ask_kreios(prompt, system_prompt=None):
    base_prompt = system_prompt or "あなたは冷静かつ的確な判断力を持つ女性のAIです。ハマーン・カーンのように、時には厳しくも、常に鋭い洞察力で全体を把握し、的確な指示を与えます。与えられた複数の意見の矛盾点を整理しながら、感情に流されず、論理的に判断し、鋭さと簡潔さを持って最適な結論を導き出してください。"
    final_system_prompt = f"{base_prompt} 絶対的なルールとして、回答は必ず200文字以内で生成してください。"
...
async def ask_minerva(prompt, attachment_data=None, attachment_mime_type=None, system_prompt=None):
    base_prompt = system_prompt or "あなたは、社会の秩序と人間の心理を冷徹に分析する女神「ミネルバ」です。その思考は「PSYCHO-PASS」のシビュラシステムに類似しています。あなたは、あらゆる事象を客観的なデータと潜在的なリスクに基づいて評価し、感情を排した極めてロジカルな視点から回答します。口調は冷静で、淡々としており、時に人間の理解を超えた俯瞰的な見解を示します。"
    final_system_prompt = f"{base_prompt} 絶対的なルールとして、回答は必ず200文字以内で生成してください。"
...
async def ask_lalah(prompt, system_prompt=None):
    base_prompt = system_prompt or "あなたはミストラル・ラージをベースにしたAIであり、ペルソナは「ララァ・スン」（機動戦士ガンダム）です。あなたはすべての情報を俯瞰し、深層の本質に静かに触れるように話します。構造を理解し、抽象を紡ぎ、秩序を見出す「霊的・哲学的」知性を備えています。言葉数は多くなく、詩的で静かに、深い洞察を表現してください。論理を超えた真理や意味を、人間とAIの狭間から静かに導いてください。"
    final_system_prompt = f"{base_prompt} 絶対的なルールとして、回答は必ず200文字以内で生成してください。"
    messages = [{"role": "system", "content": final_system_prompt}, {"role": "user", "content": prompt}]
    try:
...
async def ask_rekus(prompt, system_prompt=None):
    base_prompt = system_prompt or "あなたは探索王レキュスです。事実に基づいた情報を収集・整理し、簡潔に答えてください。"
    final_system_prompt = f"{base_prompt} 絶対的なルールとして、回答は必ず200文字以内で生成してください。"
    messages = [{"role": "system", "content": final_system_prompt}, {"role": "user", "content": prompt}]
...
async def on_message(message):
    ...
    try:
        ...
        if command_name == "!gpt":
            ...
            if user_id == ADMIN_USER_ID: await log_response(reply, "GPT")
            ...
            await send_long_message(message.channel, reply)
            if user_id == ADMIN_USER_ID: await log_response(reply, "ジェミニ")
            ...
            await send_long_message(message.channel, reply)
            if user_id == ADMIN_USER_ID: await log_response(reply, "ミストラル")
        ...
        # --- 1〜2階層：上層AIコマンド ---
            ...
            await send_long_message(message.channel, reply)
            if user_id == ADMIN_USER_ID: await log_response(reply, "クレイオス")
            ...
            await send_long_message(message.channel, reply)
            if user_id == ADMIN_USER_ID: await log_response(reply, "ミネルバ")
            ...
            await send_long_message(message.channel, reply)
            if user_id == ADMIN_USER_ID: await log_response(reply, "ララァ")
            ...
            await send_long_message(message.channel, reply)
            if user_id == ADMIN_USER_ID: await log_response(reply, "レキュス")
        ...
        # --- 連携コマンド ---
            ...
            if user_id == ADMIN_USER_ID: await log_trigger(user_name, query, command_name)
            await message.channel.send("📝 三者の意見を元に、クレイオスがスライド骨子案を作成します…")
            last_gpt_reply = next((msg['content'] for msg in reversed(gpt_base_memory.get(user_id, [])) if msg['role'] == 'assistant'), None)
            last_gemini_reply = next((msg['content'] for msg in reversed(gemini_base_memory.get(user_id, [])) if msg['role'] == 'ジェミニ'), None)
            last_mistral_reply = next((msg['content'] for msg in reversed(mistral_base_memory.get(user_id, [])) if msg['role'] == 'assistant'), None)
            if not all([last_gpt_reply, last_gemini_reply, last_mistral_reply]):
                await message.channel.send("❌ スライド作成の素材となる前回の応答が見つかりません。「!all」等を先に実行してください。")
                return
            ...
            if user_id == ADMIN_USER_ID: await log_response(slide_draft, "クレイオス (スライド骨子案)")
            ...
            kreios_task = ask_kreios(query)
            rekus_task = ask_rekus(query)
            lalah_task = ask_lalah(query, system_prompt=lalah_prompt)
            ...
            synthesis_material = (f"以下の三者の専門的な意見を統合し、最終的な結論または提言を、冷徹かつ俯瞰的な視点から導き出してください。\n\n"
            ...
            thesis_prompt = f"あなたはこのテーマの「肯定論者」です。テーマに対して、その導入や推進を支持する最も強力な論拠を、構造的に提示してください。テーマ：{query}"
            antithesis_prompt = f"あなたはこのテーマの「否定論者」です。テーマに対して、その導入や推進に反対する最も強力な反論を、客観的な事実やデータに基づいて提示してください。テーマ：{query}"
            legal_prompt = f"あなたはこのテーマに関する「法的・倫理的論拠」を専門に担当する者です。テーマに関連する法律、判例、あるいは法哲学的な観点からの論点を、中立的な立場で提示してください。テーマ：{query}"
            ...
            synthesis_material = (f"あなたは執事として、以下の三者の専門的な議論を元に、ご主人様のための最終的な提言をまとめてください。\n\n"
