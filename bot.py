    ...
    try:
        ...
        for pair in pairs:
            if ':' in pair:
                ...
                NOTION_PAGE_MAP.get(thread_id.strip()) = page_id.strip()
        ...
        # グループB：Notion参照型ナレッジAI
            ...
            if command_name == "!スライド":
                ...
                tasks = {
                    ...
                    "ミネルバ": ask_minerva(final_query, system_prompt="与えられた情報と添付ファイルを元に回答してください。", attachment_parts=[{'mime_type': attachment_mime_type, 'data': attachment_data}] if attachment_data else []),
                    "レキュス": ask_rekus(final_query + "\n\n【Notion参考情報】\n" + context)
                }
                results = await asyncio.gather(*tasks.values(), return_exceptions=True)
                synthesis_material = "以下の6つの異なるAIの意見と、外部調査の結果を統合して、プレゼンテーションのスライド骨子案を作成してください。\n\n"
                for (name, result) in zip(tasks.keys(), results):
                    ...
                    if is_admin: await log_response(target_notion_page_id, reply_text, f"{name} (!スライド_意見)")

                await message.channel.send("🌐 レキュスが外部情報を確認します…")
                external_info = await ask_rekus(final_query, system_prompt="ユーザーの質問に関する最新のWeb情報を収集・要約してください。")
                await send_long_message(message.channel, f"**🌐 外部情報 (レキュス):**\n{external_info}")
                synthesis_material += f"--- [外部情報 (レキュス)] ---\n{external_info}\n\n"
                if is_admin: await log_response(target_notion_page_id, external_info, "!スライド_外部情報")

                await message.channel.send("✨ gpt-5が最終的なスライド骨子案を作成します…")
                slide_prompt = "あなたはプレゼンテーションの構成作家です。以下の情報を統合し、聞き手の心を動かすスライド骨子案を提案してください。\n・タイトル（必須）\n・スライド1: [タイトル] - [内容]\n・スライド2: [タイトル] - [内容]\n・...\n（必要に応じてスライドを追加してください）"
                slide_draft = await ask_beast_gpt5(synthesis_material, system_prompt=slide_prompt)
            ...
            if command_name in ["!クレイオス", "!ミネルバ", "!レキュス", "!ララァ", "!Beast"]:
                ...
                elif command_name == "!ミネルバ": bot_name, reply = "ミネルバ", await ask_minerva(prompt_with_context, attachment_parts=[{'mime_type': attachment_mime_type, 'data': attachment_data}] if attachment_data else [])
                ...
                tasks = {
                    ...
                    "ミネルバ": ask_minerva(prompt_with_context, attachment_parts=[{'mime_type': attachment_mime_type, 'data': attachment_data}] if attachment_data else []),
                ...
                tasks_internal = {
                    ...
                    "中立分析官(ミネルバ)": ask_minerva(prompt_with_context, system_prompt="あなたはこの議題に関する【中立的な分析官】です。関連する社会的・倫理的な論点を、感情を排して提示してください。", attachment_parts=[{'mime_type': attachment_mime_type, 'data': attachment_data}] if attachment_data else [])
                }
                tasks_external = {"外部調査(レキュス)": ask_rekus(final_query, system_prompt="ユーザーの質問に関する最新のWeb情報を収集・要約してください。")}

                results_internal, results_external = await asyncio.gather(
                    asyncio.gather(*tasks_internal.values(), return_exceptions=True),
                    asyncio.gather(*tasks_external.values(),
