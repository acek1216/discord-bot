...
required_keys = [
    ...
    'OPENAI_API_KEY', 'GEMINI_API_KEY', 'PERPLEXITY_API_KEY', 'MISTRAL_API_KEY',
    'NOTION_API_KEY', 'ADMIN_USER_ID', 'NOTION_PAGE_ID', 'CLOUD_API_KEY'
]
...
client = discord.Client(intents=intents) # この 'client' 変数がDiscord Botの本体
...
async def ask_llama(user_id, prompt):
    ...
    try:
        ...
        return reply
    except Exception as e: return f"🛑 Llama 3.3 非同期処理エラー: {e}"
...
async def ask_claude(user_id, prompt):
    ...
    try:
        ...
        return reply
    except Exception as e: return f"🛑 OpenRouter経由 Claude 呼び出しエラー: {e}"
...
def process_message_in_background(event):
    """AIへの問い合わせと返信をバックグラウンドで行う関数 (タイムアウト対策)"""
